...
// ============== TEMPO HOLD ==============
function TempoGame({ onComplete, onBack }: { onComplete: () => void; onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setHeld(elapsed);
      if (elapsed >= TARGET) {
        setActive(false); setBest((b) => Math.max(b, elapsed));
        const xp = 120;
        pushMinigameHistory({ date: new Date().toISOString(), game: "tempo", score: Math.round(elapsed), xp });
        completeDaily(100);
  ...
  const release = () => {
    if (!active) return;
    setActive(false);
    setBest((b) => Math.max(b, held));
  };

  const pct = Math.min(100, (held / TARGET) * 100);

  return (
    <div>
      <BackBar onBack={onBack} title="Tempo Hold" />
      <div className="glass-strong" style={{ padding: 28 }}>
        <p className="text-soft mb-4" style={{ fontSize: 13 }}>Удерживай кнопку ровно {TARGET} секунд. Отпустишь раньше — сбросится.</p>
        <div style={{ position: "relative", height: 280, borderRadius: 24, overflow: "hidden", background: "rgba(148,163,184,0.15)", border: "1px solid rgba(148,163,184,0.25)" }}>
          <div style={{ position: "absolute", inset: 0, top: `${100 - pct}%`, background: "linear-gradient(180deg, #10B981, #06B6D4)", transition: "top 0.1s linear", opacity: 0.85 }} />
          <div className="absolute inset-0 flex flex-col items-center justify-center" style={{ position: "absolute" }}>
            <div style={{ fontSize: 64, fontWeight: 900, color: active ? "#fff" : "#0F172A", letterSpacing: "-0.04em", textShadow: active ? "0 4px 20px rgba(0,0,0,0.3)" : "none" }}>{held.toFixed(1)}<span style={{ fontSize: 24 }}>с</span></div>
            <div style={{ fontSize: 12, fontWeight: 700, color: active ? "rgba(255,255,255,0.85)" : "#94A3B8", letterSpacing: "0.08em", textTransform: "uppercase", marginTop: 4 }}>Лучший: {best.toFixed(1)}с</div>
          </div>
        </div>
        <button
          onMouseDown={() => { if (!finished) { setHeld(0); setActive(true); } }}
          onMouseUp={release} onMouseLeave={release}
          onTouchStart={() => { if (!finished) { setHeld(0); setActive(true); } }}
          onTouchEnd={release}
          className="w-full mt-5"
          style={{ padding: "18px", borderRadius: 16, border: "none", cursor: "pointer", background: active ? "linear-gradient(135deg, #EF4444, #F59E0B)" : "linear-gradient(135deg, #2563EB, #7C3AED)", color: "#fff", fontSize: 18, fontWeight: 800, boxShadow: "0 10px 30px rgba(37,99,235,0.35)", userSelect: "none" }}
        >
          {finished ? "Готово! +120 XP" : active ? "ДЕРЖИ..." : "Зажми и держи"}
        </button>
        {finished && (
          <button onClick={() => { setFinished(false); setHeld(0); }} className="btn-primary w-full mt-3">Ещё раз</button>
        )}
      </div>
    </div>
  );
